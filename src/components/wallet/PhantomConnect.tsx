...
export function PhantomConnect() {
  // Use individual selectors instead of a single object selector
  ...
  const attemptedAutoConnect = useWalletStore(state => state.attemptedAutoConnect);
  const lastConnected = useWalletStore(state => state.lastConnected);
  const setAttemptedAutoConnect = useWalletStore(state => state.setAttemptedAutoConnect);
  const setLastConnected = useWalletStore(state => state.setLastConnected);
  const setSession = useWalletStore(state => state.setSession);
  const reset = useWalletStore(state => state.reset);
  const network = useWalletStore(state => state.network);

  // Then destructure from adapter hooks
  const { wallet, connected, connecting, publicKey: adapterPublicKey, connect, disconnect, readyState } = useAdapterWallet();
  const { connection } = useConnection();

  // Add refs for tracking synced state
  const lastSyncedKey = useRef<string | null>(null);
  const lastSyncedSessionId = useRef<string | null>(null);

  // Compute base58Key from adapterPublicKey
